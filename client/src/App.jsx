...
function App() {
  ...
  const saveRecentUser = (user) => {
    let updatedUsers = [user, ...recentUsers.filter((u) => u !== user)];
  ...
  const searchUser = async (customUser = username) => {
    ...
    try {
      ...
      const profileRes = await axios.get(`${API_URL}/api/github/${customUser}`);
        ...
        {sortedRepos
          ...
          .map((repo) => (
            <div className="repo-card" key={repo.id}>
              ...
              >
                {expandedRepo === repo.id ? "Hide Details" : "Show Details"}
              </button>
